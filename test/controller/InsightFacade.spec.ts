...
describe("InsightFacade", function () {
	...
	describe("AddDataset", function () {
		...
		it("should successfully add more than one dataset", async function () {
			...
			const resultTwo = await facade.addDataset(
				"test123",
				sectionOne,
				InsightDatasetKind.Sections
			);
		...
		it("should reject when section does not contains every valid query keys", async function () {
			const missingQueryKey = await getContentFromArchives(
				"missingQueryKey.zip"
			);
			const result = facade.addDataset(
				"test123",
	...
	describe("PerformQuery", function () {
		...
		it(
			"[valid/simple.json] SELECT dept, avg WHERE avg > 97 ORDER by avg",
			checkQuery
		);
		it(
			"[valid/complex.json] SELECT instructor, fail WHERE avg > 98 AND NOT instructor IS f*",
			checkQuery
		);
		...
		it(
			"[valid/endWithAsterisk.json] SELECT dept, avg WHERE dept IS z*",
			checkQuery
		);
		it(
			"[valid/startWithAsterisk.json] SELECT dept, avg WHERE dept IS *y",
			checkQuery
		);
		it(
			"[valid/containString.json] SELECT dept, avg  WHERE dept IS *cnt*",
			checkQuery
		);
		it(
			"[valid/matchString.json] SELECT dept, avg  WHERE dept IS cnto",
			checkQuery
		);
		it(
			"[valid/logicAnd.json] SELECT dept, avg WHERE avg > 20 AND avg < 40",
			checkQuery
		);
		it(
			"[valid/logicOr.json] SELECT dept, avg WHERE title IS *hong* OR avg = 98",
			checkQuery
		);
		it(
			"[valid/multipleWhere.json] SELECT dept, avg WHERE avg = 98",
			checkQuery
		);
		it(
			"[valid/negationNumber.json] SELECT dept, avg WHERE NOT avg < 99",
			checkQuery
		);
		it(
			"[valid/negationString.json] SELECT fail, audit WHERE NOT dept IS *",
			checkQuery
		);
		...
		it(
			"[valid/andStringNumber.json] SELECT instructor, title WHERE dept IS *or AND year = 1900",
			checkQuery
		);
		...
		it(
			"[invalid/multipleAsterisks.json] Query with multiple asterisk",
			checkQuery
		);
		it(
			"[invalid/twoReferences.json] Query that references more than one dataset in its query keys",
			checkQuery
		);
		it(
			"[invalid/datasetNotAdded.json] Query that references a dataset that is not added",
			checkQuery
		);
		it(
			"[invalid/idStringInvalid.json] Query that uses invalid idstring",
			checkQuery
		);
		it(
			"[invalid/emptyIdString.json] Query that uses empty idstring",
			checkQuery
		);
		it(
			"[invalid/caseSensitiveIdString.json] Query that uses all caps idstring",
			checkQuery
		);
		it(
			"[invalid/mfield.json] Query that references an invalid mfield",
			checkQuery
		);
		it(
			"[invalid/sfield.json] Query that references an invalid sfield",
			checkQuery
		);
		it(
			"[invalid/mComparator.json] Query that uses an invalid MCOMPARATOR",
			checkQuery
		);
		it(
			"[invalid/logicKey.json] Query that uses an invalid LOGIC key (XOR)",
			checkQuery
		);
		it(
		...
		it(
			"[invalid/sComparison.json] Query that compare a mkey using SCOMPARISON",
			checkQuery
		);
		it(
			"[invalid/mComparison.json] Query that compare a skey using MCOMPARISON",
			checkQuery
		);
		...
		it(
			"[invalid/missingAnd.json] Query that should have AND connecting",
			checkQuery
		);
		it(
			"[invalid/orderByTwoKeys.json] Query with two keys in ORDER",
			checkQuery
		);
		it(
			"[invalid/multipleKeyGT.json] Query with multiple key in GT",
			checkQuery
		);
		it(
			"[invalid/keyTypeEQ.json] Query with invalid key type in EQ",
			checkQuery
		);
		it(
			"[invalid/structure.json] Query with a wrong structure (GT not inside WHERE)",
			checkQuery
		);
		it(
			"[invalid/optionKey.json] Query with an invalid key in OPTIONS (LT inside OPTIONS)",
			checkQuery
		);
		it(
			"[invalid/isNotObject.json] Query with IS that is not an object",
			checkQuery
		);
		it(
			"[invalid/gtNotObject.json] Query with GT that is not an object",
			checkQuery
		);
	});
});
